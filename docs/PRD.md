...
> 当前边界：Phase 5 已实现原生小程序的游客首页、企业搜索、主体确认、企业画像、动态补数、报告游客占位、“我的”和法律/帮助页；Phase 4 后端能力保持可用。Phase 6 协议、登录、诊断和报告 UI 以及 Admin 尚未实现。
...
2. 仅在用户明确同意协议并发起诊断，或主动要求查看保存报告时建立 Session。
...
- Node 自动测试与真实 HTTP 游客链路已执行；用户于 2026-08-10 在微信开发者工具完成 Phase 5 游客流程人工 E2E，所列 14 项均为 `PASS`。真机、草稿真实等待 2 小时过期、多尺寸/键盘和前后台生命周期专项检查仍为 `NOT EXECUTED`。
